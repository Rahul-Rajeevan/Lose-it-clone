...
userDayRouter.post("/",async(req,res)=>{
    ...
    jwt.verify(d, process.env.SECRET, async(err, decoded)=> {
        ...
        {  console.log(req.body);
            let p=await userDayModel.findOne({$and:[{email:decoded.email},{date:req.body.date}]})
            if(p)
            await userDayModel.updateOne({$and:[{email:decoded.email},{date:req.body.date}]},{$set:{...req.body}})
            else
           { let y=userDayModel({...req.body,email:decoded.email})
            await y.save()}
            p=await userDayModel.find({$and:[{email:decoded.email},{date:req.body.date}]})
             return  res.send(p)
            }});
    ...
    jwt.verify(d, process.env.SECRET, async(err, decoded)=> {
        ...
        t =await userDayModel.findOne({$and:[{email:decoded.email},{date:req.body.date}]})
        if(!t)
        {
